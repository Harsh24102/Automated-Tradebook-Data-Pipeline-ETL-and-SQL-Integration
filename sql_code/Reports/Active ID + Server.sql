...
ORDER BY ManagerID;
 ...
 -- To check Specific ManagerID
WITH RankedData AS (
    SELECT *, 
           ROW_NUMBER() OVER (PARTITION BY ManagerID, LEFT(ExchangeTradeTime, 8) ORDER BY ExchangeTradeTime ASC) AS rn
    FROM random_data  
    WHERE 
        CAST(SUBSTRING(ExchangeTradeTime, 10, 2) AS INT) >= 9
        AND ManagerID = 'EXPOPT40'  
)
SELECT DISTINCT LEFT(ExchangeTradeTime, 8) AS TradeDate, *  
FROM RankedData 
WHERE rn = 1
